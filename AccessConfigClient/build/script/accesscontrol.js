...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:accesscontrol.9502c613c013.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="execute",k=" ",j="Cancel",i="OK",h="T",g="middle",f="SaveUser",d='2999-12-31T23:59:59',c="DeleteUser",b="Delete Key Code",bm="Add User",bl="Yes",bk="Select New Role:",bj="All Users:",bi="Delete User",bh="Enter Key Code to check",bg="onAddRole",bf="excluded",be="Select Expiration:",bd=":   ",s="Soonest Expiration",t=" this.user.user_id: ",q="Cancel User Changes",r="qx.event.type.Data",o="GetAllUsers",p="",m="resource/nexusconfig/question.png",n="SaveUserReply",y="Set User is Done! Success: ",z="Enter New Key Code",I="AllRolesReply",F="Check Key Code",Q="DeleteUserReply",L="Add Key Code",Y="1970-01-01 00:00:00",V="changeSelection",B="Enter New Role",bc="RoleID",bb="User Name:",ba="onCheckKey",A="Role",D="Key Code:",E="Are you sure you want to delete ",H="onAddKeyValue",J="User Roles:",M="accesscontrol.UserEdit",S="cellDblclick",X="Save User Changes",u=" user_id: ",v="Enter",C="Name",P="keypress",O="?   ",N="AllUsersReply",U="Expiration",T="this.userIdx: ",K="GetAllRoles",R="Add Role",a="Delete Role",W="New User",w="New Value: ",x="ID",G="Key Hash";
qx.Class.define(M,{extend:qx.ui.container.Composite,construct:function(bn){qx.ui.container.Composite.call(this);
this.app=bn;
...
this.add(new qx.ui.basic.Label(bj),{row:0,column:0});
this.layout.setColumnWidth(1,700);
this.usersTableModel=new qx.ui.table.model.Simple();
this.usersTableModel.setColumns([C,s,x]);
...
this.usersTable.getSelectionModel().addListener(V,this.onUserSelectionChange,this);
this.usersTable.addListener(S,this.fastUserUpdate,this);
this.add(this.usersTable,{row:1,column:0});
this.addUserButton=new qx.ui.form.Button(bm);
this.delUserButton=new qx.ui.form.Button(bi);
this.add(this.addUserButton,{row:2,column:0});
this.add(this.delUserButton,{row:3,column:0});
this.addUserButton.addListener(l,this.addUser,this);
this.delUserButton.addListener(l,this.delUser,this);
...
this.user_edit_comp.add(new qx.ui.basic.Label(D),{row:2,column:0});
this.keysTableModel=new qx.ui.table.model.Simple();
this.keysTableModel.setColumns([G]);
...
this.add_key_button=new qx.ui.form.Button(L);
this.add_key_button.setMaxWidth(125);
this.add_key_button.addListener(l,this.addKey,this);
this.user_edit_comp.add(this.add_key_button,{row:4,column:0});
this.del_key_button=new qx.ui.form.Button(b);
this.del_key_button.setMaxWidth(125);
this.del_key_button.addListener(l,this.delKey,this);
this.user_edit_comp.add(this.del_key_button,{row:5,column:0});
this.check_key_button=new qx.ui.form.Button(F);
this.check_key_button.setMaxWidth(125);
this.check_key_button.addListener(l,this.checkKey,this);
this.user_edit_comp.add(this.check_key_button,{row:6,column:0});
this.user_edit_comp.add(new qx.ui.basic.Label(J),{row:2,column:1});
this.rolesTableModel=new qx.ui.table.model.Simple();
this.rolesTableModel.setColumns([A,U,bc]);
...
this.add_role_button=new qx.ui.form.Button(R);
this.add_role_button.setMaxWidth(100);
this.add_role_button.addListener(l,this.addRole,this);
this.user_edit_comp.add(this.add_role_button,{row:4,column:1});
this.del_role_button=new qx.ui.form.Button(a);
this.del_role_button.setMaxWidth(100);
this.del_role_button.addListener(l,this.delRole,this);
this.user_edit_comp.add(this.del_role_button,{row:5,column:1});
this.save_button=new qx.ui.form.Button(X);
this.cancel_button=new qx.ui.form.Button(q);
...
this.save_button.addListener(l,this.saveUser,this);
this.cancel_button.addListener(l,this.cancelSaveUser,this);
this.getAllUsers();
this.getAllRoles();
this.enable();
},events:{"SendMsg":r},members:{userIdx:-1,userHasChanged:false,onUserSelectionChange:function(){var bp=this.usersTableModel.getData();
var bo=this.usersTable.getSelectionModel().getSelectedRanges();

if(bo.length>0){console.log(bo.length+k+bo[0].minIndex+k+bp.length);

if(bo[0].minIndex<bp.length){var bq=bp[bo[0].minIndex][2];
this.userIdx=this.getUsersIndex(bq);
var br=this.users[this.userIdx];
this.setUserData(br);
}}},findUserById:function(bs){var bt;

for(bt=0;bt<this.users.length;bt++){if(this.users[bt].user_id==bs){return this.users[bt];
}}return false;
},addUser:function(){var by=new Object();
by.user_name=W;
by.user_id=this.getFreeUserID();
by.roles=new Array();
by.passwords=new Array();
var bu=new Array(by.user_name,Y,by.user_id);
var bv=this.usersTableModel.getData();
console.log(bv);
bv.push(bu);
var bx=this.usersTableModel.getSortColumnIndex();
var bw=this.usersTableModel.isSortAscending();
this.usersTableModel.setData(bv);

if(bx!=-1)this.usersTableModel.sortByColumn(bx,bw);
this.userHasChanged=true;
this.users.push(by);
this.usersTable.getSelectionModel().setSelectionInterval(bv.length-1,bv.length-1);

if(bv.length==1)this.onUserSelectionChange();
this.enable();
},saveUser:function(){this.user.user_name=this.userName.getValue();
var bA=this.keysTableModel.getData();
this.user.passwords=new Array();
var bD;

for(bD=0;bD<bA.length;bD++){this.user.passwords.push(bA[bD][0]);
}var bz=this.rolesTableModel.getData();
this.user.roles=new Array();

for(bD=0;bD<bz.length;bD++){var bC=new Object();
bC.role=bz[bD][0];
bC.role_id=bz[bD][2];
bC.expiration=bz[bD][1].replace(k,h);
this.user.roles.push(bC);
...
this.enable();
return;
}bG=this.rolesTable.getSelectionModel();

if(bG.getSelectedCount()>0){bE=bG.getSelectedRanges()[0].minIndex;
}else{alert('Please select the role you would like to delete.');
this.enable();
return;
}var bF=this.rolesTableModel.getData();
bF.splice(bE,1);
this.rolesTableModel.setData(bF);
this.userHasChanged=true;
this.enable();
...
this.enable();
return;
}bJ=this.keysTable.getSelectionModel();

if(bJ.getSelectedCount()>0){bH=bJ.getSelectedRanges()[0].minIndex;
}else{alert('Please select the hash you would like to delete.');
this.enable();
return;
}var bI=this.keysTableModel.getData();
bI.splice(bH,1);
this.keysTableModel.setData(bI);
this.userHasChanged=true;
this.enable();
},addKey:function(){this.disable();
this.collectNewName(this.add_key_button,H,z);
},checkKey:function(){this.disable();
this.collectNewName(this.check_key_button,ba,bh);
},onCheckKey:function(){var bM=accesscontrol.MD5.compute(this.nameBox.getValue());
var bN;
var bK=false;
var bL=this.keysTableModel.getData();

for(bN=0;bN<bL.length;bN++){if(bL[bN][0]==bM){alert('Key #'+(bN+1)+' matches the entered key');
bK=true;
break;
}}
if(!bK)alert('No keys matched your text');
this.cancelPopup();
this.enable();
},addRole:function(){this.disable();
this.collectNewRole(this.add_role_button,bg,B);
...
return;
}var bP=this.roleSelect.getSelection();

if(bP.length==0){alert('Please select a role');
return;
}bP=bP[0].getLabel();
var bR=this.GetRoleIDFromName(bP);
var bQ=this.dateSelect.getValue();
bQ.pushToEndOfDay();
var bO=this.rolesTableModel.getData();
bO.push(new Array(bP,bQ.toSortedString(),bR));
this.rolesTableModel.setData(bO);
this.userHasChanged=true;
this.cancelPopup();
},GetRoleIDFromName:function(name){var bT;
var bS=this.roles.length;

for(bT=0;bT<bS;bT++){if(this.roles[bT].role_name==name){return this.roles[bT].role_id;
}}return 0;
},onAddKeyValue:function(){var bU=this.keysTableModel.getData();
bU.push(new Array(accesscontrol.MD5.compute(this.nameBox.getValue())));
this.keysTableModel.setData(bU);
this.userHasChanged=true;
this.cancelPopup();
},setUserData:function(bV){this.user=bV;
this.userName.setValue(bV.user_name);
var bW=new Array();
var bY;

for(bY=0;bY<bV.passwords.length;bY++){bW.push(new Array(bV.passwords[bY]));
}this.keysTableModel.setData(bW);
var bX=new Array();

for(bY=0;bY<bV.roles.length;bY++){var ca=new Date(bV.roles[bY].expiration);
bX.push(new Array(bV.roles[bY].role,ca.toSortedString(),bV.roles[bY].role_id));
}this.rolesTableModel.setData(bX);
},getAllUsers:function(){var cc=new Object();
cc.msgType=o;
this.app.sendMsg(cc,this);
},getAllRoles:function(){var cd=new Object();
cd.msgType=K;
this.app.sendMsg(cd,this);
},onMsgReply:function(ce){if(ce.msgType==N){this.setAllUsers(ce.users);
}
if(ce.msgType==I){this.setAllRoles(ce.roles);
}
if(ce.msgType==n){this.analyzeSetUserReply(ce);
}
if(ce.msgType==Q){this.onServerDeletedUser(ce);
}},setAllRoles:function(cf){this.roles=cf;
},setAllUsers:function(cg){this.users=cg;
var cl=new Array();
var ch;

for(var cm in cg){var ci=new Date(d);

for(ch=0;ch<cg[cm].roles.length;ch++){var cn=new Date(cg[cm].roles[ch].expiration);

if(cn<ci)ci=cn;
}cl.push(new Array(cg[cm].user_name,ci.toSortedString(),cg[cm].user_id));
}var ck=this.usersTableModel.getSortColumnIndex();
var cj=this.usersTableModel.isSortAscending();
this.usersTableModel.setData(cl);

if(ck!=-1)this.usersTableModel.sortByColumn(ck,cj);
},analyzeSetUserReply:function(co){console.log(y+co.success);
},getDefaultStartDate:function(){var cp=new Date();

if(cp.getDate()>7){if(cp.getMonth()==12){cp.setMonth(2);
cp.setYear(cp.getFullYear()+1);
}else if(cp.getMonth()==11){cp.setMonth(1);
cp.setFullYear(cp.getFullYear()+1);
}else{cp.setMonth(cp.getMonth()+2);
}cp.setDate(7);
}return cp;
},findSoonestRoleIdx:function(cq){var cu;
var cr=new Date(d);
var cs=0;

for(cu=0;cu<cq.roles.length;cu++){var ct=new Date(cq.roles[cu].expiration);

if(ct<cr){cr=ct;
cs=cu;
;
}}return cs;
},onFastUpdateUser:function(){var cx=this.dateSelect.getValue();
cx.pushToEndOfDay();
var cy=cx.toSortedString();
cy=cy.replace(k,h);
var cv=this.findSoonestRoleIdx(this.user);
this.user.roles[cv].expiration=cy;
this.setAllUsers(this.users);
this.setUserData(this.user);
var cw=new Object();
cw.msgType=f;
cw.user=this.user;
this.app.sendMsg(cw,this);
this.cancelPopup();
},delUser:function(){var cA=this.usersTableModel.getData();
var cz=this.usersTable.getSelectionModel().getSelectedRanges();
var cB=cA[cz[0].minIndex][2];
this.userIdx=this.getUsersIndex(cB);
this.user=this.users[this.userIdx];
console.log(T+this.userIdx+u+cB+t+this.user.user_id);
this.askDeleteConfirmation();
},onDeleteUser:function(){var cC=new Object();
cC.msgType=c;
cC.user=new Object();
cC.user.user_id=this.user.user_id;
cC.user.user_name=this.user.user_name;
cC.user.passwords=new Array();
cC.user.roles=new Array();
this.app.sendMsg(cC,this);
},onServerDeletedUser:function(cD){if(cD.success===true){this.cancelPopup();
var cE=this.getUsersIndex(this.user.user_id);
this.users.splice(cE,1);
this.setAllUsers(this.users);
this.clearUserData();
this.onUserSelectionChange();
}else{alert("There was a problem on the server while deleting the user");
}},fastUserUpdate:function(cF){var cG=this.usersTableModel.getData();
var cJ=new String(cG[cF.getRow()][1]);
var cI=new Date(cJ.replace(k,h));
this.user=this.findUserById(cG[cF.getRow()][2]);
...
this.nameBox=new qx.ui.basic.Label(cG[cF.getRow()][0]+bd);
this.nameBox.setAlignY(g);
this.dateSelect=new qx.ui.form.DateField();
this.dateSelect.setValue(cI);
this.nameOKButton=new qx.ui.form.Button(i);
this.nameOKButton.addListener(l,this.onFastUpdateUser,this);
this.nameCancelButton=new qx.ui.form.Button(j);
this.nameCancelButton.addListener(l,this.cancelPopup,this);
...
var cH=new Object();
cH.top=cF.getDocumentTop()-15;
cH.left=10;
this.popup.placeToPoint(cH);
this.disable();
this.popup.show();
...
this.popup.setPaddingTop(15);
this.popup.setPaddingBottom(15);
this.nameBox=new qx.ui.basic.Label(E+this.user.user_name+O);
this.nameBox.setAlignY(g);
this.nameOKButton=new qx.ui.form.Button(bl);
this.nameOKButton.addListener(l,this.onDeleteUser,this);
this.nameCancelButton=new qx.ui.form.Button(j);
this.nameCancelButton.addListener(l,this.cancelPopup,this);
this.popup.add(this.nameBox);
this.popup.add(this.nameOKButton);
...
},collectNewName:function(cL,cM,cN){this.popup=new qx.ui.popup.Popup(new qx.ui.layout.HBox());
this.popup.setAutoHide(false);
this.popup.setPaddingLeft(15);
this.popup.setPaddingRight(15);
this.popup.setPaddingTop(15);
...
this.nameBox.setPlaceholder(cN);
this.nameBox.setAlignY(g);
this.nameBox.setMarginRight(3);
this.nameBox.setMarginLeft(15);
this.nameBox.addListener(P,function(e){if(e.getKeyIdentifier()==v){this[cM]();
}},this);
this.nameOKButton=new qx.ui.form.Button(i);
this.nameCancelButton=new qx.ui.form.Button(j);
this.nameOKButton.addListener(l,this[cM],this);
this.nameCancelButton.addListener(l,this.cancelPopup,this);
this.popup.add(new qx.ui.basic.Atom(w,m));
this.popup.add(this.nameBox);
this.popup.add(this.nameOKButton);
this.popup.add(this.nameCancelButton);
var cO=new Object();
...
cO.top=cP.top;
cO.left=cP.left;
cO.top+=Math.round(cP.height/2);
cO.left+=cP.width+15;
this.popup.placeToPoint(cO);
this.disable();
this.popup.show();
this.nameBox.getFocusElement().focus();
},collectNewRole:function(cQ,cR,cS){this.popup=new qx.ui.popup.Popup(new qx.ui.layout.Grid());
this.popup.setAutoHide(false);
this.popup.setPaddingLeft(15);
this.popup.setPaddingRight(15);
this.popup.setPaddingTop(15);
...
for(cW=0;cW<this.roles.length;cW++){var cT=new qx.ui.form.ListItem(this.roles[cW].role_name);
this.roleSelect.add(cT);
}this.nameOKButton=new qx.ui.form.Button(i);
this.nameCancelButton=new qx.ui.form.Button(j);
this.nameOKButton.addListener(l,this[cR],this);
this.nameCancelButton.addListener(l,this.cancelPopup,this);
this.popup.add(new qx.ui.basic.Label(bk),{row:0,column:0});
this.popup.add(new qx.ui.basic.Label(be),{row:0,column:1});
...
cU.top=cV.top;
cU.left=cV.left;
cU.top+=Math.round(cV.height/2);
cU.left+=cV.width+15;
this.popup.placeToPoint(cU);
this.disable();
this.popup.show();
this.roleSelect.getFocusElement().focus();
},cancelPopup:function(){this.popup.setVisibility(bf);
this.enable();
},getFreeUserID:function(){var cY=0;
var da;
var cX=this.usersTableModel.getData();

for(da=0;da<cX.length;da++){if(cX[da][2]>cY)cY=cX[da][2];
}return cY+1;
},getUsersIndex:function(db){var dc;

for(dc=0;dc<this.users.length;dc++){if(this.users[dc].user_id==db){return dc;
}}}}});
})();
